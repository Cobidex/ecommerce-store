...
from services.logger import (
    get_all_products_logger,
    search_product_logger,
    get_product_logger,
    product_update_logger,
    upload_product_logger,
    delete_product_logger
    )
...
def GET_all_products():
    ...
    products = [product.to_dict() for product in storage.all(Product, **querry).values()]
    if len(products) <= 0:
...
def SEARCH_products():
    ...
    if len(products) <= 0:
        search_product_logger.info(f"search: {name}, status: failed, reason: not found")
...
def POST_product(user):
    ...
    for location in locations:
        ...
        location_details['cost_of_delivery'] = int(location.get('cost_of_delivery'))
